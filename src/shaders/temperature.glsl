...
vec3 temperature(float t) {
  const vec3 c[10] = {vec3(0.0f / 255.0f, 2.0f / 255.0f, 91.0f / 255.0f),
  ...
  const float wc =
      smoothstep(float(cur) - blur, float(cur) + blur, s) *
      (1.0f - smoothstep(float(cur + 1) - blur, float(cur + 1) + blur, s));
  ...
  return vec3(clamp(r.x, 0.0f, 1.0f), clamp(r.y, 0.0f, 1.0f),
              clamp(r.z, 0.0f, 1.0f));
}
