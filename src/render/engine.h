...
class render_engine {
 ...
 private:
  ...
  bool convert_to_geometry_nv(object_model_t &object,
                              VkGeometryNV &blas_geometry) {
    blas_geometry = {};
    blas_geometry.sType = VK_STRUCTURE_TYPE_GEOMETRY_NV;
    blas_geometry.geometryType = VK_GEOMETRY_TYPE_TRIANGLES_NV;

    // Vertex.
    blas_geometry.geometry.triangles.sType =
        VK_STRUCTURE_TYPE_GEOMETRY_TRIANGLES_NV;
    blas_geometry.geometry.triangles.vertexData = object.vertex_buf;
    blas_geometry.geometry.triangles.vertexOffset = object.vertex_offset;
    blas_geometry.geometry.triangles.vertexCount =
        static_cast<uint32_t>(object.vertices.size());
    blas_geometry.geometry.triangles.vertexStride = sizeof(Vertex);
    blas_geometry.geometry.triangles.vertexFormat = VK_FORMAT_R32G32B32_SFLOAT;
    // Index.
    blas_geometry.geometry.triangles.indexData = object.index_buf;
    blas_geometry.geometry.triangles.indexOffset = object.index_offset;
    blas_geometry.geometry.triangles.indexCount =
        static_cast<uint32_t>(object.indices.size());
    blas_geometry.geometry.triangles.indexType = VK_INDEX_TYPE_UINT32;
    // Transformation
    blas_geometry.geometry.triangles.transformData = VK_NULL_HANDLE;
    blas_geometry.geometry.triangles.transformOffset = 0;

    // axis-aligned bounding box.
    blas_geometry.geometry.aabbs = {};
    blas_geometry.geometry.aabbs.sType = VK_STRUCTURE_TYPE_GEOMETRY_AABB_NV;

    blas_geometry.flags = VK_GEOMETRY_OPAQUE_BIT_NV;  // TODO: Change to zero.
  ...
  bool create_acceleration_structure(
    ...
    if (!memory_.allocate_memory(mem_req_2.memoryRequirements, properties,
                                 as.mem)) {
  ...
  bool build_blas(const std::vector<std::vector<VkGeometryNV>> &geometries,
    ...
    if (!memory_.create_buffer(max_scratch_size, usage, properties,
                               scratch_buffer, scratch_buffer_memory)) {
  ...
  bool create_blas() {
    ...
    uint32_t num_geometries = objects_.size();
    ...
    for (auto &object : objects_) {
      VkGeometryNV blas_geometry;
      if (!convert_to_geometry_nv(object, blas_geometry)) {
  ...
  bool create_tlas() {
    ...
    for (int i = 0; i < static_cast<int>(objects_instances_.size()); ++i) {
      ...
      instance.instance_id = i;  // gl_InstanceID
  ...
  bool build_tlas(const std::vector<blas_instance_t> &tlas,
    ...
    if (!memory_.create_buffer(scratch_size, usage, properties, scratch_buffer,
    ...
    if (!memory_.create_buffer_and_copy(tlas_device_size, tlas_usage,
                                        tlas_properties, tlas_.buffer,
                                        tlas_.mem, geometry_instances.data())) {
  ...
  // Ray Tracing stuff.
  //
  static constexpr bool old_rtx_ = false;
  bool rtx_enabled_;
  ray_tracer rtx_;
